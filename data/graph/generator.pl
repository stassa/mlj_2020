:-module(path_generator, [write_dataset/0
                         ,write_dataset/2
...
%
write_dataset:-
        generator_configuration:mislabelling_probability(P)
        ,generator_configuration:mislabelling_type(T)
        ,write_dataset(T,P).
...
%
write_dataset(L,P):-
        generator_configuration:background_knowledge(BK)
        ,generator_configuration:metarules(IDs)
        ,generator_configuration:target_prefix_arity(T,A)
        ,generator_configuration:output_root(Root)
        ,init_output_dir
...
%
init_output_dir:-
	generator_configuration:output_root(R)
	,(   \+ exists_directory(R)
	 ->   make_directory(R)
	 ;   true
	 ).
...
%       path_generator.pl) and the predicates in their closure and
%       writes each definition to the experiment file in Stream.
%
write_bk_definitions(S,BK):-
        closure(BK,path_generator,Bs)
        ,forall(member(Cs,Bs)
               ,(numbervars(Cs)
                ,write_terms(S,Cs)
                ,nl(S)
...
%	,path_generator:connected(A,B)),_FNs)
