...
%!  learn(+Pos,+Neg,+BK,+Metarules,-Program) is det.
%
%   Learn a Program from the given MIL problem.
%
...
%!  clause_expansion(+Metasubs,-Program) is det.
%
%   Expand a list of Metasubs to a Program.
%
clause_expansion(Prog1,Prog2):-
    reverse(Prog1,Prog3),
...
basename_examples(Bn,Pos,Neg):-
    ...
    ,findall(E_p
            ,user:pos(_,E_p)
            ,Pos)
    ,(   \+ user:predicate_property(neg(_,_), defined)
     ->  Neg = []
     ;   user:predicate_property(neg(_,_), number_of_clauses(0))
     ->  Neg = []
     ;   findall(E_n
                ,(user:neg(_,E_n)
                 )
...
proveall(Atoms,Sig,Prog):-
    ...
    debug(metagol,'learning ~w',[P/A]),
    iterator(MaxN),
    debug(metagol,'clauses: ~d',[MaxN]),
